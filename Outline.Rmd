...
tenFoldCrossVal = function(formula, n = 10, data, type, nnn=1, ...){
  ...
  for (i in 1:n){
    ...
    fit = switch(type,
                 ...
                 svm = svm(formula, data = train, ...),
                 gbm = gbm(formula, data = train, ...),
                 neunet = neuralnet(formula,data=train,hidden=c(nnn)),stepmax=3e+05,lifesign="full",...)
...
train_dum_reduced_log<-do_transform(train_dum_reduced)
...
hnn_20nodes_t<-neuralnet(nn_form,data=train_dum_scaled,hidden=c(10,10),linear.output=T,rep=1,stepmax=3e+05,lifesign="full",act.fct = "tanh")
