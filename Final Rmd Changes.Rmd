...
```{r SVM}
grid1 = expand.grid(C=10^(-3:3))
grid1 = expand.grid(C=10^(-3:3),
                    sigma = c(.001, .01, .05, .1, .15 ))
  
tuned.svm.linear=train( x = train4$SalePrice,
                        y = train4[,-c(SalePrice)],
...
tuned.svm.radial=tuned.svm.linear=train( x = train4$SalePrice,
                                         y = train4[,-c(SalePrice)],
                                         preProcess = c("center", "scale"),
                                         method = 'svmRadial',
                                         trControl = trainControl(method = 'cv',
...
After spending a lot of time tuning the SVM the best I could get was .44 crossvalidated RMSE.  I didn't think it was worth submitting on Kaggle.


```{r}

build_model <- function(train, type = 4, transformed = FALSE){
    #xgboost here
...
test$SalePrice <- predict(lm.model, test)
nrow_test <- nrow(test)
nrow_train <- nrow(train)
combined <- rbind(train, test)

combined.xgb_model <- build_model(combined, type = 4)
fitone.xgb.rmse = rmse(actual = combined$SalePrice, predicted = predict(combined.xgb_model,
                                                                        xgb.DMatrix(data=data.matrix(
                                                                          combined[,1:(ncol(combined)-1)]))))
print(fitone.xgb.rmse)
test$SalePrice <- reverse_transformed_response(
                          predict(combined.xgb_model, xgb.DMatrix(data=data.matrix(
                            test[,1:(ncol(test)-1)]))))
...
XGBoost had the best results of the methods we tried.  When submiting it to kaggle we got an RMSE of 0.1602 and placed 925
